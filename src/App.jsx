    ...
    <div style={S.container}>
      ...
      <main style={S.main}>
        {view === 'workout' ? (<>
          ...
          <div style={S.progBar}><div style={S.progHeader}><span style={S.progLabel}>WORKOUT PROGRESS</span><span style={S.progPct}>{progress()}%</span></div><div style={S.progTrack}><div style={{...S.progFill, width: `${progress()}%`}}/></div></div>
      ...
      {showNutrition && <div style={S.overlay} onClick={() => setShowNutrition(false)}><div style={S.nutritionModal} onClick={e => e.stopPropagation()}><h3 style={S.nutritionTitle}>Today's Nutrition</h3><div style={S.nutritionSection}><div style={S.nutritionSectionHeader}><span>💧 Water</span><span style={S.nutritionProgress}>{todayWater}/{waterGoal}</span></div><div style={S.waterTrack}>{[...Array(waterGoal)].map((_, i) => <div key={i} style={{...S.waterGlass, background: i < todayWater ? '#3B82F6' : 'rgba(255,255,255,0.08)'}} onClick={() => setTodayWater(i+1)}/>)}</div><button onClick={addWater} style={S.addWaterBtn}>+ ADD GLASS</button></div><div style={S.nutritionSection}><div style={S.nutritionSectionHeader}><span>🥩 Protein</span><span style={S.nutritionProgress}>{todayProtein}/{proteinGoal}g</span></div><div style={S.proteinBar}><div style={{...S.proteinFill, width: `${Math.min(100,(todayProtein/proteinGoal)*100)}%`}}/></div><div style={S.proteinGrid}>{proteinSources.map(src => <button key={src.name} onClick={() => addProtein(src)} style={S.proteinBtn}><span style={S.proteinBtnIcon}>{src.icon}</span><span style={S.proteinBtnName}>{src.name}</span><span style={S.proteinBtnVal}>+{src.protein}g</span></button>)}</div></div><button onClick={() => setShowNutrition(false)} style={S.nutritionClose}>DONE</button></div></div>}
...
function Onboarding({ onComplete }) {
  ...
  return <div style={S.obContainer}><div style={S.obDots}>{[0,1,2,3,4,5,6].map(i => <div key={i} style={{...S.obDot, background: i <= step ? '#FF3B30' : '#333'}}/>)}</div>{steps[step]}<style>{css}</style></div>;
}
...
function Dashboard({ profile, workoutHistory, exerciseHistory, todayWater, todayProtein, proteinGoal, waterGoal, nutritionHistory, personalRecords, bodyMeasurements, streak, weekCount, onAddWater, onOpenNutrition, onOpenMeasurements, onShowExerciseHistory }) {
  ...
  return (
    <div>
      ...
      <div style={S.chartBox}><h3 style={S.chartTitle}>🍽️ Today's Nutrition</h3><div style={S.nutritionCards}><div style={S.nutritionCard} onClick={onOpenNutrition}><div style={S.nutritionCardHeader}><span style={{fontSize:'1.25rem'}}>💧</span><span style={S.nutritionCardTitle}>Water</span></div><div style={S.nutritionCardValue}>{todayWater}<span style={S.nutritionCardUnit}>/{waterGoal}</span></div><div style={S.miniProgress}><div style={{...S.miniProgressFill, width: `${(todayWater/waterGoal)*100}%`, background: '#3B82F6'}}/></div><button onClick={e => { e.stopPropagation(); onAddWater(); }} style={S.nutritionCardBtn}>+ ADD</button></div><div style={S.nutritionCard} onClick={onOpenNutrition}><div style={S.nutritionCardHeader}><span style={{fontSize:'1.25rem'}}>🥩</span><span style={S.nutritionCardTitle}>Protein</span></div><div style={S.nutritionCardValue}>{todayProtein}<span style={S.nutritionCardUnit}>g</span></div><div style={S.miniProgress}><div style={{...S.miniProgressFill, width: `${Math.min(100,(todayProtein/proteinGoal)*100)}%`, background: '#34C759'}}/></div><button onClick={e => { e.stopPropagation(); onOpenNutrition(); }} style={S.nutritionCardBtn}>+ LOG</button></div></div></div>

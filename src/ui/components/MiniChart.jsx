...
export default function MiniChart({ data, color = '#FF3B30', type = 'line', height = 50 }) {
  ...
  if (type === 'bar') {
    return (
      <svg width="100%" height={height} viewBox="0 0 100 50" preserveAspectRatio="none">
        {data.map((v, i) => (
          <rect key={i} x={i * (100 / data.length) + 1} y={50 - (v / max) * 45}
            width={100 / data.length - 2} height={(v / max) * 45}
            fill={color} opacity={0.5 + i * 0.05} rx="2" />
        ))}
      </svg>
    );
  }
